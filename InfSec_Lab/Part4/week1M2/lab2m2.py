...
part = np.argpartition(np.array(times),80)
sigs_h = []
sigs_s = []
for i in range(80):
    sigs_h.append(jsons[part[i]]["h"])
    sigs_s.append(jsons[part[i]]["s"])


ts = []
us = []
for i in range(len(sigs_h)):
    h = sigs_h[i]
    s = sigs_s[i]
    h,u = hidden_number_problem(0,h,s)
    ts.append(h)
    us.append(u)
w = vector(us+[0])

expon = 2**129
p_c = math.sqrt((2**254)*81) * expon / 2

M = matrix.identity(ZZ,80) * q * expon
